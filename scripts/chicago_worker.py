...
def run(plan_dir,catalog_path,output,limit,source_parent=None,bulk=None,release_points=False,assembly=None,way_index=None,
    ...
    def geometry_job(job):
        ...
        if not world.exists():
            ...
            if not staging.exists():build(root/'sources',staging,point_source=root/'points',world_frame=plan['frame'])
            ...
            verify(staging);staging.rename(world)
    ...
    try:
        while processed<limit and not stopping:
            if shutil.disk_usage(ROOT).free<22*2**30:raise ValueError('Internal free-space reserve reached')
            ...
            try:
                ...
                else:points=crop_sources(items,grid,point_dir,compress_working=not release_points,
